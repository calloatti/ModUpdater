...
using System.Diagnostics; // Added for Process.GetProcessesByName
...
namespace ModUpdater
{
  ...
  class Program
  {
    ...
    {
      // Cross-platform fix: Linux terminals often throw exceptions if they don't support Title changes
      try { Console.Title = "ModUpdater"; } catch { }

      // 1. Check if Timberborn is running
      // Cross-platform fix: Unity Linux native builds are often named with .x86_64
      if (Process.GetProcessesByName("Timberborn").Length > 0 ||
          Process.GetProcessesByName("Timberborn.x86_64").Length > 0)
      {
        Console.WriteLine("Error: Timberborn is currently running.");
        Console.WriteLine("Please close the game to prevent file conflicts.");
        Console.WriteLine("\nPress any key to exit...");
        Console.ReadKey();
        return;
      }

      // 2. Initialize Steam API with pause on failure
      if (!SteamAPI.Init())
      {
        Console.WriteLine("SteamAPI.Init failed.");
        Console.WriteLine("Ensure Steam is running and steam_appid.txt is present.");
        Console.WriteLine("\nPress any key to exit...");
        Console.ReadKey();
        return;
      }
